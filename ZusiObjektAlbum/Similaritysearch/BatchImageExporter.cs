...
namespace ZusiObjektAlbum.Similaritysearch
{
  ...
  public static class BatchImageExporter
  {
    ...
    public static async Task ExportAndIndexAllAsync(
        ...
        ObservableCollection<ObjectModel> objectModels = null)
    {
      ...
      if (objectModels == null)
      {
        ...
        leaves = LoadLeaves(); // take Objectmodels from parameter if provided, otherwise load all leaves
      }
      else
      {
        CollectLeaves(objectModels,leaves);
      }

      progress?.Report($"{leaves.Count} Objekte gefunden.");

      //progress?.Report("Lade CLIP-ONNX-Modell...");
      //using IImageEmbedder embedder = new ClipEmbedder(modelPath);

      progress?.Report("Lade CLIP-ONNX-Modell...");
      using IImageEmbedder embedder = new Dinov2Embedder(modelPath); 
    ...
    /// </summary>
    public static void ExportAndIndexSingle(ObjectModel om, IImageEmbedder embedder, string indexPath, string? debugImageFolder = null)
    {
      EmbeddingIndex index = LoadOrCreateIndex(indexPath);
      bool didEmbed = ProcessObjectWithIndex(om, embedder, index, debugImageFolder);
      index.SaveToFile(indexPath);
    ...
    /// </summary>
    private static bool ProcessObjectWithIndex(ObjectModel om, IImageEmbedder embedder, EmbeddingIndex index, string? debugImageFolder)
    {
